...
# -----------------------------------------------------
themes_path="$HOME/.config/hypr/waybar/mlfw/themes"
...
for value in $options; do
    if [ ! $value == "$HOME/.config/hypr/waybar/mlfw/themes/assets" ]; then
        if [ ! $value == "$themes_path" ]; then
            if [ $(find $value -maxdepth 1 -type d | wc -l) = 1 ]; then
                result=$(echo $value | sed "s#$HOME/.config/hypr/waybar/mlfw/themes/#/#g")
