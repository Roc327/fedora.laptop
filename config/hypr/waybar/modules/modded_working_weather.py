...
def format_chances(hour, text: Dict[str, str]) -> str:
    ...
    val = itemgetter(1)
    sorted_probs = {
        e: probs[key(e)] for e in sorted(probs.items(), key=val, reverse=True)
    }
...
def main():
    ...
    arg_parser.add_argument(
        "-u", "--unit", help="Your unit of temperature measurement (C or F)", type=str
    )
    arg_parser.add_argument(
        "-w",
        "--windspeed-unit",
        help="Your unit of wind speed measurement (K or M)",
        type=str,
    )
    ...
    try:
        ...
        #    f"https://{config.lang}.wttr.in/{args.location}?format=j1",
        #    timeout=10,
        # ).text

        weather_json = requests.get(
            "https://en.wttr.in/Jacksonville?format=j1",
            timeout=10,
        ).text
        output = get_output(weather_json, config)
